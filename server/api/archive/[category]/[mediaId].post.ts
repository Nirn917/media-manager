...
import { arrDeleteFiles } from '~/server/lib/arr'
import type { Category } from '~/types/media'

// POST /api/archive/[category]/[mediaId]
// Verifies the backup exists in pcloud_index, then deletes the local files
// via the *arr file-level DELETE endpoint (NOT the movie/series DELETE which
// would remove the entry entirely). The media stays in Radarr/Sonarr with
...
export default defineEventHandler(async (event) => {
  ...
  try {
    await arrDeleteFiles(inst, item.id)
